...
library(readxl)
library(openxlsx)
source('Helper_addMenuSection.R')

rm(list = ls())

wd = '/Users/huangyuru/PycharmProjects/MenuTracker/March_collection_2022'

setwd(wd)

write_csv(file='MenuTracker_MarchCollection_170422(complete).csv',data_all)
data_all <-read_csv('MenuTracker_MarchCollection_140422.csv',guess_max = 20000)
...
# 2. wetherspoon
wetherspoon_file = list.files(list.files(pattern='wetherspoon',full.names=TRUE,ignore.case = TRUE),pattern='.csv',full.names = TRUE)
wetherspoon = read_csv(wetherspoon_file)
...
check_bb(starbucks)
colnames(starbucks)[7] = 'item_description'
...
pret$servingsize = pret$kcal/pret$kcal_100*100
...
colnames(nero) = c('delete','collection_date','rest_name',"item_name", "item_description", "vegeterian",
                   ...
                   "sugar_100",  "sugar", "sugar_uom", "salt_100","salt", "salt_uom", 
                   "sodium_100", "sodium", "sodium_uom", "fibre_100", "fibre", "fibre_uom")
table(nero$menu_section)
nero$servingsize = nero$kcal/nero$kcal_100*100
...
#15. wagamama 
wagamama_file = list.files(list.files(pattern='wagamama',full.names=TRUE,ignore.case = TRUE),pattern='.csv',full.names = TRUE)
wagamama = read_csv(wagamama_file)
wagamama
...
wagamama_unique$servingsize = wagamama_unique$kj/wagamama_unique$kj_100*100
...
# 17. Brewers Fayre
# clean files
addMenuSection(folder_name ='Brewers',file_n=1)
...
#18. Sizzling pubs
sizzling_file = list.files(list.files(pattern='sizzling',full.names=TRUE,ignore.case = TRUE),pattern='.csv',full.names = TRUE)
sizzling = read_csv(sizzling_file)
...
# 20. Chef and Brewer 
chef = pdf_combine('Chef',"Chef and Brewer")
check_bb(chef)
# add menu reference to menu 
chef$menu_section = gsub('\r',' ',chef$menu_section)
chef$menu_reference = gsub('\r',' ',chef$menu_reference)
chef$menu_section = paste(chef$menu_reference,chef$menu_section,sep=', ')
chef$menu_reference= NULL
data_all<-bind_rows(data_all,chef)

# 21. Table table --> has the exact same menu as the brewers fayre (!)
table = pdf_convert('table','Table Table')
check_bb(table)
data_all <- bind_rows(data_all,table)

#22.Toby
toby_file = list.files(list.files(pattern='toby',full.names=TRUE,ignore.case = TRUE),pattern='.csv',full.names = TRUE)
toby = read_csv(toby_file)
...
# 23. Revolution Bars
revolution_file = list.files(list.files(pattern='revolution',full.names=TRUE,ignore.case=TRUE),pattern='.csv',full.names = TRUE)
revolution = read_csv(revolution_file)
revolution2 = revolution  %>% mutate_at(vars('kcal','fat',
                                             'satfat','carb','protein','sodium'), function(x) as.numeric(str_extract(pattern='[0-9]+([.][0-9]+)?',gsub(pattern=',',replacement='',x))))
# delete the inaccurate rows
revolution3 = revolution2[(!is.na(revolution2$item_name))& revolution2$kcal!=0 & !is.na(revolution2$kcal),]
check_bb(revolution3)
data_all = bind_rows(data_all,revolution3)
dim(data_all)

# 24. zizzi
zizzi = pdf_convert('Zizzi',"Zizzi")
check_bb(zizzi)
data_all<-bind_rows(data_all,zizzi)


# 25. ask 
ask = pdf_convert('Ask','Ask')
check_bb(ask)
...
allbarone = read_csv(allbarone_file)
# View(allbarone)
...
# 32. Loch Fyne: no NI anymore 

# 33. PAUL
paul_file = list.files(list.files(pattern='paul',full.names=TRUE, ignore.case=TRUE),pattern='.csv',full.names = TRUE)
paul = read_csv(paul_file)
...
walkabout = read_csv(walkabout_file)
check_bb(walkabout)
...
# 58. Vue 
vue = pdf_combine('Vue','VUE ENTERTAINMENT')
check_bb(vue)
vue$servingsize = as.character(vue$servingsize)
...
# 60. Martons 
martons = pdf_convert('Marstons','Marston\'s')
check_bb(martons)
martons = clean_columns(martons)
data_all = bind_rows(data_all,martons)

# 61. Morrisons 
morrisons = pdf_convert('Morrisons','Morrisons Cafe')
check_bb(morrisons)
...
tesco$kcal = as.numeric(gsub(pattern=" calories", replacement="",tesco$kcal_c))
check_bb(tesco)
...
colnames(cornish)<-c('collection_date','rest_name','item_name','item_description',
                     'price','ingredients','allergens','servingsize','url',
...
taco_bell$transfat = as.character(taco_bell$transfat)
taco_bell$transfat_100 = as.character(taco_bell$transfat_100)
...
# NO LONGER NI: Coco
# coco = read_data('Coco')
# colnames(coco) = c('rest_name','collection_date','menu_section','url','item_name',
#                    'allergens','kj','kcal','fat','satfat','carb','sugar','fibre','protein','salt',
#                    'kj_100','kcal_100','fat_100','satfat_100','carb_100','sugar_100','fibre_100',
#                    'protein_100','salt_100')
# check_bb(coco)
# data_all = bind_rows(data_all, coco)



# NO LONGER NI:  Café Rouge 
# rouge = pdf_convert('rouge','Café Rouge')
# rouge = clean_columns(rouge)
# check_bb(rouge)
# data_all = bind_rows(data_all, rouge)

data_all2 = data_all[!duplicated(data_all),]
write_csv(path='SepCombined(Complete)_031021.csv',data_all2)
